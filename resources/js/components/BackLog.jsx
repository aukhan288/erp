...
export default function BackLog({ projectId, onTaskDrag, refreshKey }) {
  ...
  const [dragToSprint, setDragToSprint] = useState(false);
...
// Open modal for add/edit
  const openModal = (task = null, sprintId= null) => {
    
    
    if (task) {
      setEditingTask(task);
       setDragToSprint(!!sprintId);
      setFormData({
...
const handleSubmitSprint = async (e) => {
  ...
  try {
    await api.put(`/sprints/${editingSprint.id}`, sprintForm);
...
function DroppableSprint({ sprint, selectedSprint, setSelectedSprint }) {
  ...
  return (
    <div
      ...
      className={`p-2 rounded cursor-pointer border border-gray-200 
        ${isOver ? "bg-teal-700 text-white" : "bg-white"}
      `}
    ...
    >
      ...
      <br />
      <small>
        Start: {moment(sprint.start_date).format("MMMM Do YYYY")}
      </small>
      <br />
      <small>
        End: {moment(sprint.end_date).format("MMMM Do YYYY")}
      </small>
    </div>
  );
};
...
const handleDragEnd = async (event) => {
  ...
  // Only allow drop on sprint
  if (!String(over.id).startsWith("sprint-")) return;

  const taskId = active.id;
  const sprintId = over.id.replace("sprint-", "");

  const sprint = sprints.find((b) => String(b.id) === String(sprintId));
  const task = backlogs.find((b) => String(b.id) === String(taskId));

  if (!task || !sprint) return;

  // check if sprint info is incomplete
  if (!sprint.start_date || !sprint.end_date) {
    Swal.fire({
      icon: "warning",
      title: "Incomplete Sprint",
      text: "This sprint does not have full information. Please update it first.",
    });
    return;
  }

  // open modal with backlog data
  openModal(task, sprintId);
};
...
function DraggableBacklogCard({
  ...
  : backlog.title}
  <br />
    <span className="text-gray-700 text-xs">  {backlog?.sprint?.name.length > 70
  ? backlog?.sprint?.name.substring(0, 70) + "..."
  : backlog?.sprint?.name}</span>
        </div>
...
>
    <div className="p-4 border border-gray-300 rounded-lg bg-white shadow-sm">
      ...
      <h2 className="text-2xl font-bold mb-4 text-teal-700 ">Backlogs</h2>
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        {/* BACKLOG (7 columns) */}
        <div className="col-span-9 bg-gray-100 p-4 rounded-lg min-h-[400px]">
      ...
      {sprintModal &&
        (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
            <h4 className="text-lg font-semibold mb-4">Edit Sprint</h4>
             <form onSubmit={handleSubmitSprint} className="space-y-3 max-h-[500px] overflow-y-auto">
               
              <div>
                <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">Sprint Name<span className="text-rose-800">*</span> </label>
                <input type="text" name="name" value={sprintForm.name} onChange={handleSprintChange} required
                  className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 border-gray-300 focus:ring-teal-500" />
              </div>
              <div>
                <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">Goal<span className="text-rose-800">*</span></label>
                <textarea name="goal" value={sprintForm.goal} onChange={handleSprintChange} required
                  className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 border-gray-300 focus:ring-teal-500" />
              </div>
              <div className="flex gap-2">
                <div className="flex-1">
                  <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">Start Date<span className="text-rose-800">*</span></label>
                  <input type="date" name="start_date" value={sprintForm.start_date} onChange={handleSprintChange} required
                    className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 border-gray-300 focus:ring-teal-500"  />
                </div>
                <div className="flex-1">
                  <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">End Date<span className="text-rose-800">*</span></label>
                  <input type="date" name="end_date" value={sprintForm.end_date} onChange={handleSprintChange} required
                    className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 border-gray-300 focus:ring-teal-500"  />
                </div>
              </div>
              <div className="flex justify-end gap-2 mt-4">
                <button type="button" className="px-3 py-1 border rounded hover:bg-gray-100" onClick={() => setSprintModal(false)}>Cancel</button>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 overflow-y-auto max-h-[80vh] mb-5 pb-5 mt-5">
              {/* Title */}
              <div>
                <label className="block mb-1 font-medium text-gray-700">Title<span className="text-rose-800">*</span></label>
                <input
                  type="text"
                  name="title"
                  className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 border-gray-300 focus:ring-teal-500"
                ...
                />
                    ...
                    <div>
                <label className="block mb-1 font-medium text-gray-700">Description{dragToSprint && <span className="text-rose-800">*</span>}</label>
                <textarea
                  name="description"
                  required={dragToSprint}
                  className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 border-gray-300 focus:ring-teal-500"
               ...
               {/* Description */}
              <div>
                <label className="block mb-1 font-medium text-gray-700">Acceptance Criteria{dragToSprint && <span className="text-rose-800">*</span>}</label>
                <textarea
...
<div>
  ...
  <div className="col-span-12 md:col-span-4">
    <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">
      Priority{dragToSprint && <span className="text-rose-800">*</span>}
    </label>
  ...
  <div className="col-span-12 md:col-span-8">
    <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">
      Estimated Time (minutes){dragToSprint && <span className="text-rose-800">*</span>}
    </label>
    ...
    />
              ...
              {/* Due Date */}
              <div>
                <label className="block mb-1 font-medium text-gray-700 dark:text-gray-200">Due Date{dragToSprint && <span className="text-rose-800">*</span>}</label>
                <input
                  type="date"
                  required={dragToSprint}
                  name="due_date"
            ...
            <div className="space-y-3 text-gray-700 dark:text-gray-300">
              ...
              {viewTask.acceptance_criteria && (
                <div>
                  ...
                  <p className="text-gray-700 dark:text-gray-300">{viewTask.acceptance_criteria}</p>
                </div>
              )}
