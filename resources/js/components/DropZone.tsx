...
accept: {
      ...
      >
        ...
        <div className="dz-message flex flex-col items-center">
          ...
          <span className="text-center mb-3 block w-full max-w-[290px] text-sm text-gray-700 dark:text-gray-400">
            {isDragActive ? "Drop files here" : "Drag and drop here or browse"}
          </span>
  ...
  <div className="mt-3 space-y-2">
    {files.map((file, i) => (
      <div
        key={i}
        className="flex items-center justify-between bg-gray-100 dark:bg-gray-800 px-3 py-2 rounded"
      >
        <span className="text-sm">{file.name}</span>

       <button
  onClick={() => handleDelete(file.id, i)}
